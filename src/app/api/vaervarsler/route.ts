...
function lagVarsler(dager: VærDag[]): { tittel: string; melding: string; type: string }[] {
  const varsler: { tittel: string; melding: string; type: string }[] = []
  ...
  if (erSårbarSesong && iMorgen.min <= 2 && !varsler.find(v => v.type === 'frost')) {
    varsler.push({
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Lagre varsler i varsel_logg
    for (const varsel of varsler) {
      await supabase.from('varsel_logg').insert({
